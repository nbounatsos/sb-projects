...
# print(read_dir('Data/DSSP_files_small_lib', read_AccUnfold()))
...
def print_propensities(all_aa_count, buried_aa_count, outfile):
    ...
    # print(all_aa_count.values())
    N_total = sum(all_aa_count.values())
    N_total_s = sum(buried_aa_count.values())
    for aa in list_aa:
        N_aa = all_aa_count[aa]
        N_aa_s = buried_aa_count[aa]
        propensity_buried = ( (N_aa_s) / (N_aa) ) / ( (N_total_s) / (N_total) )
